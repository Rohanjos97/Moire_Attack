...
examples_merge_from_raw_files_with_post_demosaicing.ipynb>`__.
    ...
    H_RB = tf.constant(H_RB)
    CFA = CFA.unsqueeze(1)

    R = tf.nn.conv2d(CFA * R_m, H_RB, strides=[1, 1, 1, 1], padding='SAME')
    G = tf.nn.conv2d(CFA * G_m, H_G, strides=[1, 1, 1, 1], padding='SAME')
    B = tf.nn.conv2d(CFA * B_m, H_RB, strides=[1, 1, 1, 1], padding='SAME')
